class Api::V1::ChangelogsController < ApplicationController
  ...
  def index_by_project
    changelogs = Changelog.joins("LEFT OUTER JOIN employees AS emps ON emps.id=changer_id").select("changelogs.*, emps.name as changer_name").where(project_id: params[:id]).order(:change_date)
